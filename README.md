...
All data provided with this repository is synthetic (generated by Google Gemini), to comply with Data Protection rules.
...
The experiments reported in the report were trained using AWS Sagemaker GPU instances. Training can also be performed on a local machine with a compatible GPU.

Training the machine learning models is computationally expensive and is recommended to be performed with a CUDA-enabled NVIDIA GPU. The remaining components, including data preprocessing, inference, evaluation, and the visualiser, can be run on a CPU-only machine.
...
| `7_create_splits.py` | Creates train, validation, and test splits using multi-label stratification and group-leakage prevention. | ✅ |
...
```bash
python src/spans/eval/predict_*.py
```
