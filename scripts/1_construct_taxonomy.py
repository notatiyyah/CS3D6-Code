...
# 2. Load and clean original CSV
df = pd.read_csv(INPUT_PATH, usecols=['Person Attribute Category', 'Description (attribute, 40 chars)', 'Value (of data type)'])
df.dropna(inplace=True)

df.columns = ['high_level_category', 'category_description', 'values_hint']
df['values_hint'] = df['values_hint'].str.split("\n")

# 3. Convert labels to snake_case
cat_labels = []
...
df['cat_label'] = cat_labels

# 4. Join with regexes and export
...
if len(regex_df) != len(df):
    print("ERROR: Mismatch between autogenerated labels and category regex definition. Please check.")
