...
export default function DocumentPanel({ title, spans, relationMap, text, hoveredSpanId, setHoveredSpanId }) {
    ...
    const chunks = useMemo(() => {
        ...
        // 1. Collect every unique boundary (start or end) across all spans.
        ...
        // 2. Walk consecutive boundary pairs; each pair is a segment
    ...
    return (
        <div className="panel">
            ...
            <div className="document-view">
                {chunks.map((chunk) => {
                    // Loop through each text 'chunk' and display as html spans
                    if (!chunk.isEntity) {
                    ...
                    const primaryStyle = getEntityStyle(primary.type);
                    ...
                    // Stack one underline bar per covering span
                    const boxShadow = chunk.spans
                        .map((s, i) => `0 ${2 + i * 3}px 0 0 ${getEntityStyle(s.type).border}`)
                        .join(', ');
